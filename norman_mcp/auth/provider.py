...
class NormanOAuthProvider(OAuthAuthorizationServerProvider):
    ...
    async def register_client(self, client_info: OAuthClientInformationFull) -> None:
        ...
        URI must be on the server allow-list so an attacker cannot persist a
        code-exfiltration target. The authoritative check still runs per
        /authorize request via validate_redirect_uri.
        """
        dropped = [
            str(uri)
