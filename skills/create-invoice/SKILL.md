...
- Use `update_invoice` for invoice/quote edits and `update_recurring_invoice` for schedule edits. Send only changed fields in `changes`. Keep existing line IDs when editing items. A document design replaces the saved design, so read and include its other controls when changing only one control. Explicit false, zero, blank strings and valid nulls are different from omitted fields.
