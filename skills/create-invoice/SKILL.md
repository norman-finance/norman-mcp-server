...
- To bill the same thing again, call `duplicate_invoice` with the invoice or quote id: it makes a draft dated today with the same lines and payment term. Move the service or delivery dates with `update_invoice`, then set status "saved" to issue it. A draft is never emailed or matched to a payment.
